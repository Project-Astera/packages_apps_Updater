...
updates_dir=/data/astera_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have ProjectAstera-VERSION-DATE-TYPE-* as format"
...
adb shell "killall com.astera.updater 2>/dev/null"
adb shell "sqlite3 /data/data/com.astera.updater/databases/updates.db" \
